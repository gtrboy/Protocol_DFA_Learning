...
public class PktIKEAuthCert extends PktIKEEnc{
    ...
    public byte[] getPlaintext(Element plainRoot, int dataLen) {
        ...
        Element eleCert = plainRoot.element("payload_cert");
        ...
        bb.put(ParseCertPayload(eleCert));
    ...
    private byte[] ParseCertPayload(Element certRoot){
        ByteArrayOutputStream bAos = new ByteArrayOutputStream();
        Element pHdr = certRoot.element("p_header");
        Element pData = certRoot.element("data");
        byte[] hdr = ParsePayloadHdr(pHdr, pData);
        bAos.writeBytes(hdr);

        for(Iterator it = pData.elementIterator(); it.hasNext();) {
            Element element = (Element) it.next();
            if (element.getName().equals("cert_url")){
                String url = element.getText();
                bAos.writeBytes(url.getBytes(StandardCharsets.UTF_8));
            }else {
                try {
